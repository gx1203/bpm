      ...
      <div class="bas-components-header-content">
        <div class="logo">
          <img
            ...
            @click="goHome"
            :src="require(`@/bas/assets/img/login/logo-white_jiucaiyun.png`)"
          >
        </div>
        <!-- <el-popover
          v-model="navShow"
        ...
        <div v-if="$route.path !== '/doorCenter'" class="mainnav">
          <i
        ...
        <div class="right-side">
          <span v-if="$route.path !== '/doorCenter'" class="workbench" @click="goDoorCenter">
            <img
              :src="require(`@/assets/img/common/icon-home.png`)"
            >
            个人工作台
          </span>
          <!-- <switch-language /> -->
          ...
          >
            <img
              :src="require(`@/assets/img/common/headPortrait.png`)"
            >
            {{ $t('welcome2') }}， {{ username }}
          </div>
          <div
...
export default {
    ...
    $route: {
      handler(route) {
        ...
        for (let i = 0, len = this.userMenu.length; i < len; i++) {
          ...
          if (
            subNodes.map(el => el.url && el.url.split('?')[0]).includes(route.name) ||
            subNodes.map(el => el.url && el.url.split('?')[0]).includes(route.fullPath)
          ) {
  ...
  methods: {
    ...
    ...mapMutations(['setCarouselShow', 'setActiveIndex']),
