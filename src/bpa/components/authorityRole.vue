<template>
  <el-dialog :title="selectInfo.name + '权限配置'" class="tree_list" :visible.sync="dialogVisible" width="70%" :close-on-click-modal = false :close-on-press-escape = false>
...
import { getMenuList, addAuthorityToOrg, addAuthorityToRole, addButtonAuthorityToOrg,
  deleteButtonAuthorityFromOrg, addButtonAuthorityToRole, deleteButtonAuthorityFromRole, getMenuAndButton } from '@/bpa/api/settings/menusManage'
...
export default {
  ...
  methods: {
    ...
    getMenuList () {
      ...
      // })
      // 获取选中的组织树的数据权限
      getMenuAndButton(this.menuTreeParams).then(res => {
        this.menusTree = res
        this.grandParentId = []
        this.selectId(res)
      })
