<template>
  ...
  >
    <slot name="prv"></slot>
    <el-table-column v-if="useIndexColumn" type="index" width="50" label="序号"></el-table-column>
    <!-- <el-table-column
      fixed
      label="名称"
      min-width="200"
        ...
        >
          <template slot-scope="scope">
            <el-popover placement="top-start" width="200" trigger="hover">
              <div>
                <p>计划开始时间：{{scope.row.startDate}}</p>
                <p>计划结束时间：{{scope.row.endDate}}</p>
                <p>实际开始时间：{{scope.row.realStartDate}}</p>
                <p>实际结束时间：{{scope.row.realEndDate}}</p>
              </div>
              <div slot="reference">
...
<script>
import dayjs from 'dayjs' // 导入uuid生成插件
...
import {
  ...
  flattenDeepParents,
  regDeepParents
} from '@/bpa/utils/array' // 导入日期js
const uuidv4 = require('uuidv4')
dayjs.extend(isBetween) // 导入数组操作函数
