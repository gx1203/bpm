<template>
  <el-dialog
    class="addKpiIndicators"
    :title="
      editItem.id
        ? existTabField('editIndicators', '', true)
        : existTabField('addIndicators', '', true)
    "
  ...
  >
    ...
    >
      ...
      <div class="setExpress-wrapper">
        <el-form-item class="set-item" :label="$t('KPIValues')" required>
          <el-input
            disabled
            type="textarea"
            autosize
            v-model="targetform.exname"
            resize="none"
          ></el-input>
        </el-form-item>
        <el-button class="setExpress" @click="setexpressFlag = true">{{
          $t("SetFormula")
        }}</el-button>
      </div>
    ...
    <span slot="footer" class="dialog-footer">
      <el-button @click="Suk_cancel">{{ $t("cancel") }}</el-button>
      <el-button type="primary" @click="confirmProcess">{{
        $t("confirm")
      }}</el-button>
    </span>
    <setFormulaDialog
      v-model="setexpressFlag"
      @confirm="getFormulaValue"
      :kpiValue="this.targetform.exname"
    />
  </el-dialog>
</template>
<script>
import { saveTarget } from "@/bpa/api/processDesignCenter";
...
export default {
  ...
  methods: {
    ...
    confirmProcess() {
      let self = this;
      console.log(self.targetform);
      this.$refs.processRef.validate(async (valid) => {
        if (valid) {
          if (self.targetform.exname) {
            let form = JSON.parse(JSON.stringify(self.targetform));
            console.log(form);
