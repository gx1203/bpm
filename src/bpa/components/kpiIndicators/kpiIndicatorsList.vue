        ...
        <template slot-scope="scope">
          ...
          >
            <el-button
              ...
              type="text"
              icon="el-icon-edit"
              @click="addKpiIndicators('edit', scope.row)"
            ></el-button>
          </el-tooltip>
          <el-tooltip
            class="item"
...
export default {
  ...
  created() {
    if (this.$props.kpitype == "all") {
      this.getShowTablist()
      this.getShowTabField()
      this.loading = true;
      getprocessValue({
        filters: [{ key: "listid", value: "0" }],
        orderby: "createon",
        pageNum: 1,
        pageSize: 10,
        sort: "desc",
  ...
  methods: {
    ...
    getData() {
      getListIdByModelId(this.curSelectedId).then((rt) => {
        if (rt) {
          this.modelid = rt;
          this.getValuelist();
        }
