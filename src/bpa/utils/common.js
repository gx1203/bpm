 ...
 * 返回类型 2018-01-01
 */
export const getFormateDate = (date, format) => {
  // 兼容IE
  var time
  // console.log(date)
  if (!date) return ''
  if (typeof date === 'string') {
    // console.log(date.replace(/-/g, '/').replace(/T|Z/g, ' ').trim().slice(0, 19), new Date(date.replace(/-/g, '/').replace(/T|Z/g, ' ').trim()), new Date(date.replace(/-/g, '/')), new Date(date))
    time = new Date(date.replace(/-/g, '/').replace(/T|Z/g, ' ').trim().slice(0, 19))
...
export function viewFile(fileid) {
  ...
  return process.env.VUE_APP_HOST_URL + process.env.VUE_APP_BASE_URL + '/document/print/' + fileid + '?username=' + userId
  // return 'http://test5.jiucaiyun.cn/bpabpmapi/document/print/' + fileid + '?username=' + userId
}
...
export function downloadFile(fileid) {
  const userId = localStorage.getItem('portalCurrentUserInfo') ? JSON.parse(localStorage.getItem('portalCurrentUserInfo')).empuid : ''
  console.log(process.env.VUE_APP_HOST_URL + process.env.VUE_APP_BASE_URL + '/regulation/download/' + fileid + '?username=' + userId)
  return process.env.VUE_APP_HOST_URL + process.env.VUE_APP_BASE_URL + '/regulation/download/' + fileid + '?username=' + userId
  // return 'http://test5.jiucaiyun.cn/bpabpmapi/document/print/attach/download/' + fileid + '?username=' + userId
}
/**
 * 下载文件
 */
 export function downloadFiles(fileid) {
  let userId = localStorage.getItem('portalCurrentUserInfo') ? JSON.parse(localStorage.getItem('portalCurrentUserInfo')).empuid : ''
