  ...
  <el-container id="dicenter" class="di__container">
    <el-main class="mainChart">
      ...
      <el-row :span="24" class="targetChart">
        <el-col :span="16" class="button-base">
          ...
          <ul class="w100 h100">
            ...
            <li class="w100 h33 cursor-pointer relative" @mouseover.stop="showvaries.child3 = true" @mouseout.stop="showvaries.child3 = false">
              ...
              <ul class="showContent" v-show="showvaries.child3">
                ...
                <li class="fontplan2"><span>{{$t("plannedValue1")}}:</span>{{!!childrenChartId[2] ? childrenChartId[2].pvalue2:''}}</li>
      ...
      <el-row class="h37">
        <el-col :span="23" :offset="1" class="di__echart-item h100">
          <el-col class="h18 title vertical-middle">
            ...
            <span class="order-item1" @click="orderClick" v-if="!orderPD">{{$t("afterFive")}}<i class="el-icon-sort-down"></i></span>
          </el-col>
          <el-col class="h75">
      ...
      <el-row class="h37">
        <el-col :span="23" :offset="1" class="di__echart-item h100">
          <el-col class="h18 title vertical-middle">
            ...
            <span class="order-item1" @click="orderClick1" v-if="!orderPD1">{{$t("afterFive")}}<i class="el-icon-sort-down"></i></span>
          </el-col>
          <el-col class="h75">
  ...
  methods: {
    ...
    async getDashboard () {
      ...
      let res = await Promise.all([
        ...
        profitDashboard(self.month)
      ])
      console.log(res)
      this.childrenChartId = res[0]
      this.profitChartdata = res[1]
      let profitMax = res[1].pvalue1 > res[1].kpiValue ? (res[1].pvalue1 + Math.pow(10, (res[1].pvalue1 + '').length - 1)) : (res[1].kpiValue + Math.pow(10, (res[1].kpiValue + '').length - 1))
      ...
      self.profitChart.data = [{'value': (res[1].kpiValue * 100 / profitMax).toFixed(2)}]
      self.profitChart.detail.formatter = res[1].kpiValue / ((profitMax + '').length < 4 ? 1 : 10000) + '\n ' + this.$t("pretaxProfits") + ((profitMax + '').length < 4 ? '' : ' (万)')
      ...
      for (let i = 0; i < res[0].length; i++) {
        ...
        obj.max = profitMax
        obj.silent = false
        obj.detail.formatter = value / ((profitMax + '').length < 4 ? 1 : 10000) + '\n' + name + ((profitMax + '').length < 4 ? '' : ' (万)')
        obj.data = [{'value': parseFloat(item.kpivalue).toFixed(2), 'id': item.id}]
      ...
      console.log(arr)
      console.log(self.childrenChartId)
      console.log(series[0].data[0].value)
      series[0].data[0].value = 6.7
      console.log(series[0].data[0].value)
      console.log(series)
      // self.$nextTick(() => {
      let firstChild = echarts.init(self.$refs.centerChart)
      setInterval(function () {
        series[0].data[0].value = (Math.random() * 100).toFixed(2) - 0
