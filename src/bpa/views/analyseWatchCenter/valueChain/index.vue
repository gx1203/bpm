<template>
  <div id="edgeToedge" class="w100" @click="rightContextMenu = false">
      ...
      <el-col class="remove-margin-r">
        <div :class="{'fullScreen': isFull, '': !isFull}" class="paintChart">
          <div class="tools">
            ...
            <el-button size="mini" @click="newStage">{{$t("addPhase")}}</el-button>
            <el-button size="mini" @click="autoSort">{{$t("automaticSorting")}}</el-button>
            <!-- <el-button style="float:right " @click="$router.push('/analyseWatchCenter/bpmProcessBack')">{{$t("return")}}</el-button> -->
            <el-tooltip
              class="margin-l10 cursor-pointer close-btn"
              style="float:right " 
              effect="dark"
              :content="$t('closes')"
              placement="top"
            >
              <i class="el-icon-close" aria-hidden="true" @click="$router.push('/analyseWatchCenter/bpmProcessBack')"/>
    ...
    <!-- 添加价值链 -->
    <el-dialog :visible.sync="dialogVisible" :title="valueChain" width="40%">
    ...
    <!-- 添加阶段 -->
    <el-dialog :visible.sync="stagedialogVisible" :title="phase" width="40%">
    ...
    <ul v-show="rightContextMenu" ref="rightMenuRef" class="rightMenu">
      <li @click="modifyStage">{{$t("edit")}}</li>
