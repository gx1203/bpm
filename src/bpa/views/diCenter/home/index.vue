  ...
  <el-container id="dicenter" class="di__container">
    <el-main class="mainChart">
      <el-row :span="24">
        <el-col :span="6" class="di__router-container">
          ...
          >
            ...
            <span class="absolute header-database">{{$t("operationAndMonitoring")}}</span>
          </router-link>
        </el-col>
        <el-col :span="6" class="di__router-container">
          <router-link :to="{name: 'databaselist'}" class="di__router">
            <img :src="routerImg" alt="图片" class="w100">
            <span class="absolute header-database1">{{$t("indicatorsOfLibrary")}}</span>
          </router-link>
        </el-col>
        <el-col :span="6" class="di__router-container">
          <router-link :to="{name: 'home'}" class="di__router">
            <img :src="routerImg" alt="图片" class="w100" />
            <span class="absolute header-database">{{$t("riskControl")}}</span>
          </router-link>
        </el-col>
        <el-col :span="6" class="di__router-container">
      ...
      <el-row :span="24" class="targetChart">
        <el-col :span="12" class="button-base">
          <el-button :class="{'active': month ==='LastMonth'}" @click="lastMonth">{{$t("lastMonth")}}</el-button>
          <el-button :class="{'active': month ==='ThisMonth'}" @click="newMonth">{{$t("thisMonth")}}</el-button>
          <el-button :class="{'active': month ==='ThisSeason'}" @click="seasonFilter">{{$t("thisQuarter")}}</el-button>
          <el-button :class="{'active': month ==='ThisYear'}" @click="yearFilter">{{$t("thisYear")}}</el-button>
        </el-col>
      </el-row>
      <el-row :span="24" class="dashboard">
          ...
          <ul class="w100 h100">
            <li class="w100 h33 cursor-pointer relative" @mouseover.stop="showvaries.child1 = true" @mouseout.stop="showvaries.child1 = false">
              ...
              <ul class="showContent" v-show="showvaries.child1">
                <li class="fontplan1"><span>{{$t("plannedValue1")}}:</span>{{!!childrenChartId[0] ? childrenChartId[0].pvalue1:''}}</li>
                <li class="fontplan2"><span>{{$t("plannedValue2")}}:</span>{{!!childrenChartId[0] ? childrenChartId[0].pvalue2:''}}</li>
                <li class="fontthreshold"><span>{{$t("thresholdValue")}}:</span>{{!!childrenChartId[0] ? childrenChartId[0].threshold:''}}</li>
              </ul>
            </li>
            <li class="w100 h33 cursor-pointer relative" @mouseover.stop="showvaries.child3 = true" @mouseout.stop="showvaries.child3 = false">
              ...
              <ul class="showContent" v-show="showvaries.child3">
                <li class="fontplan1"><span>{{$t("plannedValue1")}}:</span>{{!!childrenChartId[2] ? childrenChartId[2].pvalue1:''}}</li>
                <li class="fontplan2"><span>{{$t("plannedValue2")}}:</span>{{!!childrenChartId[2] ? childrenChartId[2].pvalue2:''}}</li>
                <li class="fontthreshold"><span>{{$t("thresholdValue")}}:</span>{{!!childrenChartId[2] ? childrenChartId[2].threshold:''}}</li>
              </ul>
            </li>
            <li class="w100 h34 cursor-pointer relative" @mouseover.stop="showvaries.child5 = true" @mouseout.stop="showvaries.child5 = false">
              ...
              <ul class="showContent" v-show="showvaries.child5">
                <li class="fontplan1"><span>{{$t("plannedValue1")}}:</span>{{!!childrenChartId[4] ? childrenChartId[4].pvalue1:''}}</li>
                <li class="fontplan2"><span>{{$t("plannedValue2")}}:</span>{{!!childrenChartId[4] ? childrenChartId[4].pvalue2:''}}</li>
                <li class="fontthreshold"><span>{{$t("thresholdValue")}}:</span>{{!!childrenChartId[4] ? childrenChartId[4].threshold:''}}</li>
              </ul>
            </li>
          </ul>
        </el-col>
        <el-col :span="12" class="h100 relative">
          ...
          <ul class="showContent centerpos" v-show="showvaries.centerChild">
            <li class="fontplan1"><span>{{$t("plannedValue1")}}:</span>{{profitChartdata.pvalue1}}</li>
            <li class="fontplan2"><span>{{$t("plannedValue2")}}:</span>{{profitChartdata.pvalue2}}</li>
            <li class="fontthreshold"><span>{{$t("thresholdValue")}}:</span>{{profitChartdata.threshold}}</li>
          </ul>
        </el-col>
        <el-col :span="6" class="h100">
          <ul class="h100">
            <li class="w100 h33 cursor-pointer relative" @mouseover.stop="showvaries.child2 = true" @mouseout.stop="showvaries.child2 = false">
              ...
              <ul class="showContent" v-show="showvaries.child2">
                <li class="fontplan1"><span>{{$t("plannedValue1")}}:</span>{{!!childrenChartId[1] ? childrenChartId[1].pvalue1:''}}</li>
                <li class="fontplan2"><span>{{$t("plannedValue2")}}:</span>{{!!childrenChartId[1] ? childrenChartId[1].pvalue2:''}}</li>
                <li class="fontthreshold"><span>{{$t("thresholdValue")}}:</span>{{!!childrenChartId[1] ? childrenChartId[1].threshold:''}}</li>
              </ul>
            </li>
            <li class="w100 h33 cursor-pointer relative" @mouseover.stop="showvaries.child4 = true" @mouseout.stop="showvaries.child4 = false">
              ...
              <ul class="showContent" v-show="showvaries.child4">
                <li class="fontplan1"><span>{{$t("plannedValue1")}}:</span>{{!!childrenChartId[3] ? childrenChartId[3].pvalue1:''}}</li>
                <li class="fontplan2"><span>{{$t("plannedValue2")}}:</span>{{!!childrenChartId[3] ? childrenChartId[3].pvalue2:''}}</li>
                <li class="fontthreshold"><span>{{$t("thresholdValue")}}:</span>{{!!childrenChartId[3] ? childrenChartId[3].threshold:''}}</li>
              </ul>
            </li>
            <li class="w100 h34 cursor-pointer relative" @mouseover.stop="showvaries.child6 = true" @mouseout.stop="showvaries.child6 = false">
              ...
              <ul class="showContent" v-show="showvaries.child6">
                <li class="fontplan1"><span>{{$t("plannedValue1")}}:</span>{{!!childrenChartId[5] ? childrenChartId[5].pvalue1:''}}</li>
                <li class="fontplan2"><span>{{$t("plannedValue2")}}:</span>{{!!childrenChartId[5] ? childrenChartId[5].pvalue2:''}}</li>
                <li class="fontthreshold"><span>{{$t("thresholdValue")}}:</span>{{!!childrenChartId[5] ? childrenChartId[5].threshold:''}}</li>
              </ul>
            </li>
          </ul>
        </el-col>
    ...
    <el-aside class="di_right di_echart-container" width="500px">
      <el-row class="h25">
        ...
        <el-col :span="11" :offset="1" class="di__echart-item h100">
          <el-col class="h18 title vertical-middle">{{$t("riskControlPoint")}}</el-col>
          <el-col class="h75 pos-relative">
            <div ref="Child2" class="h100" />
            <div class="showCount">
              <p>{{$t("riskControlPoint")}}</p>
              <p>{{riskControlPoint}}</p> 
            </div>
          </el-col>
        </el-col>
      </el-row>
      <el-row class="h37">
        <el-col :span="23" :offset="1" class="di__echart-item h100">
          <el-col class="h18 title vertical-middle">
            <span class="order-item">{{$t("processEfficiency")}}</span>
            <span class="order-item1" @click="orderClick" v-if="orderPD">{{$t("beforeFive")}}<i class="el-icon-sort-up"></i></span>
            <span class="order-item1" @click="orderClick" v-if="!orderPD">{{$t("afterFiveFive")}}<i class="el-icon-sort-down"></i></span>
          </el-col>
          <el-col class="h75">
      ...
      <el-row class="h37">
        <el-col :span="23" :offset="1" class="di__echart-item h100">
          <el-col class="h18 title vertical-middle">
            <span class="order-item">{{$t("processUtilization")}}</span>
            <span class="order-item1" @click="orderClick1" v-if="orderPD1">{{$t("beforeFive")}}<i class="el-icon-sort-up"></i></span>
            <span class="order-item1" @click="orderClick1" v-if="!orderPD1">{{$t("afterFiveFive")}}<i class="el-icon-sort-down"></i></span>
          </el-col>
          <el-col class="h75">
...
export default {
  ...
  data() {
    return {
      ...
      procurementOfSurplus:this.$t("procurementOfSurplus"),
      financialBalance:this.$t("financialBalance"),
      salesProfit:this.$t("salesProfit"),
      perCapitaSalesOfTheWholeFactory:this.$t("perCapitaSalesOfTheWholeFactory"),
      tender:this.$t("tender"),
      assetLife:this.$t("assetLife"),
      tenThousand:this.$t("tenThousand"),
    }
  },
  computed: {
    processPublished() {
  ...
  methods: {
    ...
    getKpi() {
      getKPI().then(res => {
        if (!res) return false
        this.KPIIndexNumber = res
      })
    },
    getRiskIndexNum(){
       getRiskIndexNum().then(res => {
        if (!res) return false
         this.riskControlPoint = res
      })
    },
    getEfficiency() {
      getEFFiciency({
    ...
    async getDashboard () {
      ...
      let res = [[
        {
          'kpiname': this.procurementOfSurplus,
          'pvalue1': 90,
          'pvalue2': 70,
          'threshold': 40,
        ...
        {
          'kpiname': this.perCapitaSalesOfTheWholeFactory,
          'pvalue1': 70,
          'pvalue2': 50,
          'threshold': 30,
          'kpivalue': parseInt(Math.random() * 70)
      ...
      self.profitChart.detail.formatter = res[1].kpiValue + this.pretaxProfits + ' ('+ self.tenThousand +')'
