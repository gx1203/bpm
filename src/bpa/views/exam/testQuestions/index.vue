  ...
  <div id="testDetail">
    <el-row>
      <el-col :span="6">
        ...
        >
          <el-button slot="append" type="primary" @click="queryTestPapers">{{$t('testSystem.search')}}</el-button>
        </el-input>
      </el-col>
      <el-button type="primary" size="small" class="float-r margin-r10" @click="addTest">{{$t('testSystem.CreateTestPaper')}}</el-button>
    </el-row>
    <el-table :data="roleList" border class="main_tab">
      ...
      <el-table-column prop="belongSubject" :label="$t('testSystem.PaperType')" />
      <!-- <el-table-column prop="codeName" label="编号及名称" /> -->
      <el-table-column prop="totalScore" :label="$t('testSystem.total')" />
      <el-table-column prop="passScore" :label="$t('testSystem.QualifiedCutoffline')"/>
      <!-- <el-table-column prop="description" label="所属类型" /> -->
      <el-table-column prop="cnname" :label="$t('creator')" />
    ...
    <el-dialog :title="testTitle" :close-on-click-modal="false" :visible.sync="testPaperAdd" :before-close="handleClose" width="62%">
      ...
      >
        <el-card class="box-card" shadow="never">
          <div slot="header" class="clearfix">
            <span>{{$t('testSystem.TestInfo')}}</span>
          </div>
            ...
            <el-row>
              <el-col :span="20">
                <el-input
                  ...
                  :placeholder="$t('testSystem.PleaseEnterTheNameOfThePaper')"
                />
              </el-col>
            </el-row>
          </el-form-item>
          <el-form-item :label="$t('testSystem.NumberAndName')" prop="codeName">
            <el-row>
              <el-col :span="20">
          ...
          <el-form-item :label="$t('testSystem.PaperType')" prop="subjectId">
            <el-row>
              <el-col :span="20">
                <el-select
                  v-model="testPaperForm.subjectId"
                  :placeholder="$t('testSystem.PleaseSelectTheTypeOfPaper')"
                >
                  <el-option
          ...
          <el-form-item :label="$t('testSystem.QualifiedCutoffline')"  prop="passScore">
            <el-row>
              <el-col :span="20">
                <el-input
                  v-model="testPaperForm.passScore"
                  :placeholder="$t('testSystem.PleaseEnterTheQualifyingScoreLine')"
                />
              </el-col>
            </el-row>
          </el-form-item>
        ...
        <el-card class="box-card" shadow="never" style="margin-top: 24px">
          <div slot="header" class="clearfix">
            <span>{{$t('testSystem.ItemInformation')}}</span>
          </div>
          <el-row>
            <el-col
                  ...
                  <el-form-item :label="'选项' + (indexs + 1)">
                    <el-row>
                      <el-col :span="18">
                        <el-input
                          v-model="items.name"
                          :placeholder="$t('testSystem.PleaseEnterOptions')"
                        />
                      </el-col>
                      <el-col
                        :span="2"
                ...
                <el-col v-if="item.type !== '3'" :span="14" :offset="1">
                  <el-form-item>
                    <el-button
                      type="primary"
                      icon="el-icon-plus"
                      @click="optionAdd(index)"
                    >{{$t('testSystem.NewOptions')}}</el-button
                    >
                  </el-form-item>
                </el-col>
                <el-col v-if="item.type !== '3'" :span="14" :offset="1">
                  <el-form-item :label="$t('testSystem.referenceVersion')" :prop="'questions.'+index+'.answer'" :rules="otherRules.answer">
                    <el-select
                     clearable
                      v-model="item.answer"
                      :multiple="item.type === '2' || item.type === 2"
                      :placeholder="$t('testSystem.PleaseEnterTheReferenceAnswers')"
                    >
                      <el-option
                        v-for="(itema, indexa) in item.option"
                        :key="indexa"
                        :label="$t('testSystem.option') + (indexa + 1)"
                        :value="$t('testSystem.option')  + (indexa + 1)"
                      />
                    </el-select>
                  </el-form-item>
                </el-col>
                <el-col v-if="item.type === '3'" :span="14" :offset="1">
                  <el-form-item :label="$t('testSystem.referenceVersion')"  :prop="'questions.'+index+'.answer'" :rules="otherRules.answer">
                    <el-input
                      ...
                      v-model="item.answer"
                      type="textarea"
                      :placeholder="$t('testSystem.PleaseEnterTheReferenceAnswers1')"
                    />
                  </el-form-item>
                </el-col>
                <el-col :span="14" :offset="1">
                  <el-form-item :label="$t('testSystem.TheQuestionAcores')" style="margin-top:10px;"  :prop="'questions.'+index+'.singleScore'" :rules="otherRules.singleScore">
                    <el-input
                    clearable
                    type="number"
                ...
                <el-button
                  ...
                  @click="
                    testPaperForm.diy
                      ...
                      : queryQuestions()
                  "
                >{{$t('testSystem.NewQuestions')}}</el-button
                >
              </el-form-item>
            </el-col>
          </el-row>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        >{{$t('testSystem.confirm')}}</el-button
        >
      </span>
    </el-dialog>
    <!-- 试题新增选择 -->
    <el-dialog :visible.sync="queryQuestionsList" width="62%" :title="$t('testSystem.ItemList')" append-to-body>
      <el-row>
        <el-col :span="24">
          <el-table
            ref="multipleTable"
          ...
          >
            ...
            <el-table-column :label="$t('bpa_number')" type="index" width="50px" />
            <el-table-column prop="questionTitle" :label="$t('testSystem.topic')" />
            <el-table-column prop="type" :label="$t('testSystem.topicType')">
              <template slot-scope="scope">
                <el-row>
                  <el-col
                    :span="20"
                  >{{ scope.row.type === 1 ? $t('testSystem.MultipleChoice') : scope.row.type === 2 ? $t('testSystem.multiple') : $t('testSystem.custom')  }}</el-col>
                </el-row>
              </template>
            </el-table-column>
            <el-table-column prop="questionContent" :label="$t('testSystem.option')" />
            <el-table-column prop="answer" :label="$t('testSystem.referenceVersion')"/>
            <el-table-column prop="createTime" :label="$t('createTime')" />
          </el-table>
          <el-pagination background
            :total="pagess.total"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="queryQuestionsList = false;selectArray=[]">{{$t('testSystem.cancel')}}</el-button>
        <el-button type="primary" @click="selectTest">{{$t('testSystem.confirm')}}</el-button>
      </div>
    </el-dialog>
    <!-- 题库 -->
    <el-dialog :visible.sync="addQuestionsType" width="42%" :title="$t('testSystem.NewQuestions')" append-to-body>
      <el-row>
        <el-col :span="24">
          <el-form label-position="right" label-width="92px" style="margin:24px 0">
            <el-form-item :label="$t('testSystem.questionType')">
              <el-select v-model="questionsType" :placeholder="$t('testSystem.selectquestionType')">
                <el-option :label="$t('testSystem.MultipleChoice')" value="1" />
                <el-option :label="$t('testSystem.multiple')" value="2" />
                <el-option :label="$t('testSystem.custom')"  value="3" />
              </el-select>
            </el-form-item>
          </el-form>
        </el-col>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="addQuestionsType = false">{{$t('testSystem.cancel')}}</el-button>
        <el-button type="primary" @click="addQuestionsTypes()">{{$t('testSystem.confirm')}}</el-button>
      </div>
    </el-dialog>
    <!-- 试题新增选择 -->
    <el-dialog :visible.sync="addQuestions" width="65%" :title="$t('testSystem.TheNewPaper')" append-to-body>
      <el-row>
        <el-col :span="24">
          <el-form label-position="right" label-width="92px" style="margin:24px 0">
            <el-form-item :label="$t('testSystem.TheNewWay')">
              <el-select v-model="questionsMode" :placeholder="$t('testSystem.PleaseSelectAddMode')">
                <el-option :label="$t('testSystem.GetItFromTheQuestionBank')" value="1" />
                <el-option :label="$t('testSystem.CustomAdd')"  value="2" />
              </el-select>
            </el-form-item>
          </el-form>
        </el-col>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="addQuestions = false">{{$t('testSystem.cancel')}}</el-button>
        <el-button type="primary" @click="addQuestionsState()">{{$t('testSystem.confirm')}}</el-button>
      </div>
    </el-dialog>
    <!-- 试卷详情 -->
    <el-dialog :visible.sync="questionsDetail" width="65%" :title="$t('testSystem.TheExaminationPaperDetails')" append-to-body class="testDetailDialog">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="questionsDetail = false">{{$t('testSystem.cancel')}}</el-button>
        <el-button type="primary" @click="questionsDetail = false">{{$t('testSystem.confirm')}}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      testTitle: this.$t('testSystem.TheNewPaper'),
      testobj: {},
      logobj: {
        tableName: "考试试卷管理",
        tableId: "",
      ...
      rules: {
        testName: [
          { required: true, message: this.$t('testSystem.PleaseEnterTheNameOfThePaper')+'！', trigger: 'blur' }
        ],
        subjectId: [
          { required: true, message: this.$t('testSystem.PleaseSelectTheTypeOfExamination')+'！', trigger: 'change' }
        ],
        // codeName: [
        //   { required: true, message: '请输入编码及名称！', trigger: 'blur' }
        // ],
        passScore: [
          { required: true, message: this.$t('testSystem.PleaseEnterAPassingScore')+'！', trigger: 'blur' }
        ]
      },
      otherRules:{
         answer: [
          { required: true, message: this.$t('testSystem.PleaseRequiredAnswer'), trigger: 'blur' }
        ],
        singleScore:[
          { required: true, message: this.$t('testSystem.PleaseTheQuestionAcores'), trigger: 'blur' }
        ],
      },
      selectArray: [],
      roleList: []
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (obj.questions.length > 0) {
            ...
            console.log('this.isCreateTest',this.isCreateTest);
            if (this.isCreateTest) {
              ...
              if (this.testTitle == "新增试卷") {
                createTest(obj).then((res) => {
                  this.testPaperAdd = false;
                  this.$message({
                    message: this.$t('testSystem.CreateTestPaperSuccessfully')+"!",
                    type: "success",
                  });
                  this.logobj.instid = "";
                ...
                console.log('obj.testId',obj.testId);
                obj.id = obj.testId;
                console.log(obj, "修改试卷");
                editQuestions(obj).then((res) => {
                  this.testPaperAdd = false;
                  this.$message({
                    message: this.$t('testSystem.SuccessInRevisingTestPaper')+"!",
                    type: "success",
                  });
                  this.logobj.instid = ""; 
              ...
              this.$message({
                message: this.$t('testSystem.PleaseFillInTheTstInformation')+'！!',
                type: 'warning'
              })
            }
          } else {
          ...
          this.$message({
                message: this.$t('testSystem.PleaseEnterTheNecessaryPaperInformation')+'！!',
            type: 'warning'
          })
          return false
        }
