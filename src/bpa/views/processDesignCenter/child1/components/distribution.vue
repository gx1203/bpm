...
export default {
    ...
    return {
      ...
      settings: {
        async: {
          ...
          // url: 'http://localhost:9527/bpabpmdevapi' + `/select/user/cqp/getTotalOrg1?username=` + userId,
        },
        check: {
      ...
      searchSettings: {
        ...
        callback: {
          onNodeCreated: self.onSearchNodeCreated,
  ...
  computed: {
    ...
    // checkData() {
    //   console.log(this.orginalChecktion)
  ...
  watch: {
    ...
    orginalChecktion: {
      handler: function(val) {
        console.log("val", val);
        this.checkData = this.orginalChecktion
          .map((item) => {
            return item.name;
          })
          .join(",");
  ...
  methods: {
      ...
      if (this.filterText) {
        ...
        abstractSearch({ orgname: this.filterText }).then((res) => {
          ...
          if (this.orgType === "distribution") {
            this.sendState = this.disData.bpmDto.orgstate
              ? this.disData.bpmDto.orgstate
              : "{\"orglist\":[]}";
            this.orginalChecktion = this.disData.bpmDto.distribution
              ? JSON.parse(this.disData.bpmDto.distribution).orglist
              : [];
          } else if (this.orgType === "orgscope1") {
            this.sendState = this.disData.bpmDto.orgstate1
              ? this.disData.bpmDto.orgstate1
              : "{\"orglist\":[]}";
            this.orginalChecktion = this.disData.bpmDto.orgscope1
              ? JSON.parse(this.disData.bpmDto.orgscope1).orglist
              : [];
          } else if (this.orgType === "orgscope2") {
            this.sendState = this.disData.bpmDto.orgstate2
              ? this.disData.bpmDto.orgstate2
              : "{\"orglist\":[]}";
            this.orginalChecktion = this.disData.bpmDto.orgscope2
              ? JSON.parse(this.disData.bpmDto.orgscope2).orglist
              : [];
          } else if (this.orgType === "orgscope3") {
            this.sendState = this.disData.bpmDto.orgstate3
              ? this.disData.bpmDto.orgstate3
              : "{\"orglist\":[]}";
            this.orginalChecktion = this.disData.bpmDto.orgscope3
              ? JSON.parse(this.disData.bpmDto.orgscope3).orglist
              : [];
          }
          ...
          this.checkData = this.orginalChecktion
            ...
            .join(",");
          const storeTempArr = self.sendState
            ? JSON.parse(self.sendState).orglist
            : [];
          ...
          if (nodes.length > 0) {
            for (var i = 0; i < nodes.length; i++) {
              if (nodes[i].type === "3") nodes[i].chkDisabled = true;
    ...
    grequireTree() {
      // console.log(this.disData.bpmDto.orgstate)//undefined
      // console.log(this.disData.bpmDto.distribution)//{"orglist":[{"id":"14246","name":"中车唐山","pname":"","pid":""}]}
      // const self = this;
      getGroupTreeList({
        pid: 0,
        type: "0,1,2,4"
      }).then((res) => {
        console.log("===========:", res); //中山唐车 、欧冶云商
        console.log("+++++", this.disData.bpmDto.orgstate);
        ...
        const storeTempArr = this.sendState
          ? JSON.parse(this.sendState).orglist
          : [];
        for (var j = 0; j < storeTempArr.length; j++) {
          if (res[0].id === storeTempArr[j].id) {
            if (storeTempArr[j].halfChecked) {
    ...
    onNodeCreated(event, treeId, treeNode) {
      const self = this;
      if (treeNode.type === "3") treeNode.chkDisabled = true;
    ...
    onSearchNodeCreated(event, treeId, treeNode) {
      const self = this;
      if (treeNode.type === "3") treeNode.chkDisabled = true;
    ...
    lookForFatherBeCheck(treeNode) {
      if (treeNode) {
        ...
        if (zTree.getNodesByParam("id", treeNode.id, null)[0].type === "0") {
          ...
          return this.lookForFatherBeCheck(
            zTree.getNodesByParam("id", treeNode.pid, null)[0]
          );
        }
      }
    },
    onCheck(event, treeId, treeNode) {
      ...
      if (
        !treeNode.checked &&
        !zTree.getNodesByParam("id", treeNode.pid, null).length
      ) {
        // console.log(1111111111)
        // let index = self.orginalChecktion
        //   .map((item) => item.id)
        //   .indexOf(treeNode.id);
        // if (index !== -1) {
        //   self.orginalChecktion.splice(index, 1);
        // }
        self.orginalChecktion = [];
        // console.log(self.orginalChecktion)
        return false;
      }
      ...
      for (var i = 0; i < zTree.getCheckedNodes().length; i++) {
        if (
          zTree.getCheckedNodes()[i].type === "0" &&
          treeNode.id === zTree.getCheckedNodes()[i].id
        ) {
        ...
        // console.log(self.orginalChecktion)
        // if (
        //   zTree.getCheckedNodes()[i].type !== "1" &&
        //   zTree.getCheckedNodes()[i].type !== "0"
        // ) {
      ...
      if (tempArr.length === 1 && tempArr[0].type === "0") {
        tempArr[0].children.forEach((item) => {
          dataId.push(item.id);
          dataName.push(item.name);
        });
      } else {
      ...
      // 搜索的时候初始化
      // this.disData.bpmDto.orgstate = JSON.stringify({ 'orglist': [{
      ...
      //   'pid': 0
      // }, ...concatData] })
      // this.disData.bpmDto.distribution = JSON.stringify({ 'orglist': [{
      //   id: dataId.join(','),
      //   name: '(' + dataName.join(',') + ')',
      //   pname: '',
      //   pid: ''
      ...
      for (var idx = 0; idx < tempArr.length; idx++) {
        ...
        if (
          ((!tempArr[idx].getCheckStatus().half && !tempArr[idx].halfCheck) ||
            tempArr[idx].halfCheck) &&
          tempArr[idx].checked
        ) {
          temp.push(objTemp);
        }
      ...
      // console.log('666666666666', self.orginalChecktion)
        ...
        if (tempIds.length > 1) {
          for (var m = 0; m < tempIds.length; m++) {
            if (!zTree.getNodesByParam("id", tempIds[m], null).length) {
              var idsArr = self.referId(
                storeArr,
                JSON.parse(JSON.stringify(self.orginalChecktion[i]))
              );
              for (var t in idsArr) {
                obj[t] = idsArr[t];
              }
      ...
      for (var key in tempObj) {
        ...
        tempObj[key].name =
          tempObj[key].name == tempObj[key].pname
            ? tempObj[key].name.join(",")
            : tempObj[key].pname + "(" + tempObj[key].name.join(",") + ")";
        tempObjArr.push(tempObj[key]);
      }
      // console.log('tempObjArr', tempObjArr)
    ...
    onSearchCheck(event, treeId, treeNode) {
      let self = this;
      let zTree = $.fn.zTree.getZTreeObj("searchOrganizationTree");
      let tempArr = [];
      let arr = [];
      let initArr = {};
      let sortArr = [];
      ...
      // console.log(arr, initArr)
      tempArr = JSON.parse(JSON.stringify(arr));
      if (initArr.type === "0") {
        self.orginalChecktion = [];
        self.orginalChecktion.push({
          id: initArr.id,
          name: initArr.name,
          pname: "",
          pid: ""
      ...
      // console.log(tempArr)
      let zTree2 = $.fn.zTree.getZTreeObj("chooserOrganizationTree");
      let tempArr2 = [];
      let arr2 = [];
      let initArr2 = {};
      // 选中该节点的部分
      for (var ii = 0; ii < zTree2.getCheckedNodes().length; ii++) {
        ...
        if (
          zTree2.getCheckedNodes()[ii].type !== "1" &&
          zTree2.getCheckedNodes()[ii].type !== "0"
        ) {
    ...
    lookForFather(value) {
      ...
      const self = this;
      if (
        zTree.getNodesByParam("id", value.pid, null)[0].type === "1" ||
        zTree.getNodesByParam("id", value.pid, null)[0].type === "0"
      ) {
        return {
          pid:
            zTree.getNodesByParam("id", value.pid, null)[0].type === "1"
        ...
        return self.lookForFather(
          zTree.getNodesByParam("id", value.pid, null)[0]
        );
      }
    },
    ...
    loopDelFather(value, arr) {
      ...
      if (!zTree.getNodesByParam("id", value.id, null).length) {
        ...
        !!zTree.getNodesByParam("id", value.id, null).length &&
        !zTree.getNodesByParam("id", value.id, null)[0].checked
      ) {
        return true;
      } else {
        return false;
    ...
    referId(storeArr, deleteArr) {
      ...
      deleteArr.name =
        deleteArr.name == deleteArr.pname
          ? [deleteArr.name]
          : deleteArr.name
            .substring(0, strlength)
            .split(deleteArr.pname + "(")[1]
            .split(",");
    ...
    confirmChoose() {
      ...
      for (var m = 0; m < storeArr.length; m++) {
        ...
        if (
          !!checkNode.length &&
          (checkNode[0].halfCheck || checkNode[0].checked)
        ) {
          tempStore.push(storeArr[m]);
        } else if (
          !checkNode.length &&
          self.loopFatherConfirm(storeArr[m], storeArr)
        ) {
          tempStore.push(storeArr[m]);
        }
      }
