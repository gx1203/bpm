    ...
    <el-dialog :append-to-body="true" class="distribution" :visible.sync="isDialogsVisible" :title="selectProcess"
      ...
      <el-row :gutter="20">
        <el-col :span="12" class="tree_wrap">
          <el-row>
            <el-input :placeholder="pleaseEnterTheContent" :title="pleaseEnterTheContent"
              class="search_component remove-margin-b" v-model="keyWords" clearable @keyup.13.native="filterTree">
  ...
  data () {
    ...
    return {
      ...
      props: {
        label: 'name',
        disabled (data, node) {
          if (node.level === 1 || data.id === self.modelId || data.isVisible === '0') {
            return true
          }
          return false
        }
      ...
      searchProps: {
        label: 'name',
        disabled (data, node) {
          if (data.id === self.modelId || data.isVisible === '0') {
            return true
          }
          return false
        }
