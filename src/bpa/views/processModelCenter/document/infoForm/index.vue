    ...
    <el-row :gutter="20">
      ...
      <el-col :span="19">
        ...
        <el-table :data="documentList" class="main_tab" stripe border>
          ...
          <el-table-column prop="poname" width="100" :label="$t('processResponsibility')"></el-table-column>
          ...
          <el-table-column prop="filename" :label="$t('fileType')"></el-table-column>
...
export default {
  ...
  methods: {
    getList (id) {
      let obj = {
        filters: [
          { key: 'filename', opt: 'LIKE', type: 'S', value: '信息表单' }
        ],
      ...
      getDocumentList(obj).then(res => {
        ...
        res.list.forEach(item => {
          ...
          //   console.log(item)
          // })
          // this.$set(item, 'superiorPerson', item.upmanagerlist.map(item => item.value).join(','))
          item.superiorPerson = item.upmanagerlist.map(item => item.value).join(',')
        })
      })
    },
