<template>
  <div v-if="isDialogsVisible" id="handbookFiles" class="w100">
    <el-dialog
      :append-to-body="true"
      width="1000px"
      :visible.sync="isDialogsVisible"
      title="选择专业"
      :close-on-click-modal="false"
      :close-on-press-escape="false"
    >
      <el-row :gutter="10">
        <el-col :span="6">
          <el-scrollbar class="left_tree_wrapper">
            <el-tree
              ref="tree"
              class="filter-tree elTree"
              :props="props"
              node-key="id"
              :load="getFilterTreeList"
              :default-expanded-keys="expandedList"
              :expand-on-click-node="false"
              lazy
              @node-click="nodeClick"
            >
              <div slot-scope="{ node, data }" class="custom-tree-node">
                <p class="grow" :title="node.label">
                  {{ node.label }}
                </p>
                <!-- <p v-if="node.level == 1 || node.level == 2" class="grow" :title="node.label">
                  {{ node.label }}
                </p>
                <p v-else class="grow" :title="node.label">
                  {{ node.data.code + node.label }}
                </p> -->
              </div>
            </el-tree>
          </el-scrollbar>
        </el-col>
        <el-col :span="18">
          <!--<el-scrollbar class="rightPart">-->
          <el-row>
            <el-col :span="12">
              <el-input
                v-model="keyWords"
                :placeholder="$t('majorname')"
                :title="$t('standardNumber')"
                class="search_component"
                clearable
                @keyup.13.native="getList"
              >
                <el-button
                  slot="append"
                  type="primary"
                  @click="
                    pages.pageNum = 1;
                    getList();
                  "
                  >{{ $t("search") }}</el-button
                >
              </el-input>
            </el-col>
          </el-row>
          <!-- 信息表格 -->
          <el-table
            v-if="!isMore"
            :data="booksList"
            class="main_tab"
            stripe
            :header-cell-style="{ background: '#E1EAFD !important' }"
            highlight-current-row
            @current-change="handleCurrentChange"
          >
            <el-table-column :label="$t('number')" type="index" />
            <el-table-column
              :label="$t('majorname')"
              prop="specname"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('majordes')"
              prop="specdesc"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('TheFinalClassificationOfTheText')"
              prop="foldername"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('LevelOfClassification')"
              prop="oneLeverFolderName"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('remarks')"
              prop="remark"
              show-overflow-tooltip
            />
            <el-table-column label="选择" width="60" align="center">
              <template slot-scope="scope">
                <el-radio
                  v-model="selectIndex"
                  :label="scope.$index"
                  class="radioEmpty"
                >
                  {{ "" }}
                </el-radio>
              </template>
            </el-table-column>
          </el-table>
          <el-table
            v-else
            :data="booksList"
            class="main_tab"
            ref="multipleTable"
            stripe
            :header-cell-style="{ background: '#E1EAFD !important' }"
            highlight-current-row
            @selection-change="handleSelectionChange"
          >
            <el-table-column type="selection" width="55" fixed align="center" />
            <el-table-column :label="$t('number')" type="index" />
            <el-table-column
              :label="$t('majorname')"
              prop="specname"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('majordes')"
              prop="specdesc"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('TheFinalClassificationOfTheText')"
              prop="foldername"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('LevelOfClassification')"
              prop="oneLeverFolderName"
              show-overflow-tooltip
            />
            <el-table-column
              :label="$t('remarks')"
              prop="remark"
              show-overflow-tooltip
            />
          </el-table>
          <el-pagination
            style="margin-top: 10px"
            :current-page.sync="pages.pageNum"
            :page-sizes="[10, 15, 20]"
            :page-size="pages.pageSize"
            background
            layout="prev, pager, next, jumper, total"
            :total="pages.total"
            @size-change="handleSizeChange"
            @current-change="handleCurrentChange"
          />
          <!--</el-scrollbar>-->
        </el-col>
      </el-row>
      <div style="margin-top: 10px">
        <span
          v-for="(item, index) in selectedAll"
          :key="index"
          class="listItem"
        >
          {{ item.specname }}
          <i @click="deleteSelectedItem(index)" class="el-icon-close" />
        </span>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="isDialogsVisible = false">{{
          $t("cancelAction")
        }}</el-button>
        <el-button type="primary" @click="submit">{{
          $t("confirmButton")
        }}</el-button>
      </span>
    </el-dialog>
  </div>
</template>

<script>
import {
  getBooksList,
  getRuleIframeTree,
  queryEnterpriseStandardFolderTreeId,
} from "@/bpa/api/rulesCenter/handbookFiles";
import paginations from "@/bpa/mixins/pagination_mixin";
import fileOperation from "@/bpa/mixins/fileDownView";
import { getFormateDate } from "@/bpa/utils/common";
import {
  repositoryget,
  repositorySpecpage,
  repositorybosave,
  repositorydelete,
  repositorySpecsave,
} from "@/bpa/api/resourcelibraryCenter";

export default {
  mixins: [paginations, fileOperation],
  props: {
    value: {
      type: Boolean,
      default: false,
    },
    isMore: {
      type: Boolean,
      default: true,
    },
    enterpriseStandardSelected: {
      type: Array,
      default() {
        return [];
      },
    },
  },
  data() {
    return {
      props: {
        label: "foldername",
        children: "childList",
        isLeaf: function (data, node) {
          return !data.parent;
        },
      },
      modelid: "",
      booksList: [],
      expandedList: [],
      keyWords: "",
      type: "",
      pages: {
        pageSize: 10,
        pageNum: 1,
        total: 0,
      },
      isDialogsVisible: false,
      multipleSelection: [],
      selectedAll: [],
      selectIndex: -1,
      iechoEcho: false,
    };
  },
  watch: {
    value(val) {
      this.isDialogsVisible = val;
      if (val) {
        this.iechoEcho = true;
        this.selectedAll = JSON.parse(
          JSON.stringify(this.enterpriseStandardSelected)
        );
      } else {
        this.pages.pageNum = 1;
        this.pages.total = 0;
        this.booksList = [];
        if (this.isMore) {
          this.$refs.multipleTable.clearSelection();
        }
      }
    },
    isDialogsVisible(val) {
      this.$emit("input", val);
    },
  },
  mounted() {},
  methods: {
    deleteSelectedItem(index) {
      this.iechoEcho = true;
      this.booksList.forEach((e, i) => {
        if (e.id === this.selectedAll[index].id) {
          this.$refs.multipleTable.toggleRowSelection(e, false);
        }
      });
      this.iechoEcho = false;
      this.selectedAll.splice(index, 1);
      this.$forceUpdate();
    },
    nodeClick(node, data) {
      this.modelid = data.data.id;
      this.getList();
    },
    async getFilterTreeList(node, resolve) {
      const id = node.data ? node.data.id : "12";
      getRuleIframeTree(id).then((res) => {
        if (!res) return false;
        if (id === "12") {
          this.modelid = res[0].id;
          this.$refs.tree.setCurrentKey(res[0].id);
          this.expandedList = [res[0].id];
          this.getList();
        }
        resolve(res);
      });
    },
    // 单选
    handleCurrentChange(val) {
      if (val) {
        this.selectIndex = this.booksList.findIndex(
          (item) => item.id === val.id
        );
      }
    },
    // 多选
    handleSelectionChange(val) {
      if (!this.iechoEcho) {
        this.multipleSelection = val;
        this.mergeSelected();
      }
    },
    mergeSelected() {
      let ids = this.multipleSelection.map((item) => item.id);
      let selectedAllIds = this.selectedAll.map((item) => item.id);
      this.multipleSelection.forEach((val) => {
        if (selectedAllIds.indexOf(val.id) === -1) {
          //如果list不存在选中的则新增
          this.selectedAll.push(val);
        }
      });
      //获取未选中的listid
      let listIds = this.booksList
        .filter((item) => !ids.includes(item.id))
        .map((item) => item.id);
      //获取未选中listid对应的obj
      let checkList = this.selectedAll.filter(
        (item) => !listIds.includes(item.id)
      );
      this.selectedAll = checkList;
    },
    selectedCheck() {
      this.iechoEcho = true;
      this.$refs.multipleTable.clearSelection();
      this.$nextTick(() => {
        this.booksList.forEach((item) => {
          this.selectedAll.forEach((v) => {
            if (item.id === v.id) {
              if (this.$refs.multipleTable) {
                this.$refs.multipleTable.toggleRowSelection(item, true);
              }
            }
          });
        });
        this.iechoEcho = false;
        this.$forceUpdate();
      });
    },
    getList() {
      const obj = {
        orderby: "createon",
        pageNum: this.pages.pageNum,
        pageSize: this.pages.pageSize,
        sort: "desc",
        filters: [
          {
            key: "specname",
            opt: "LIKE",
            type: "S",
            value: this.keyWords.trim(),
          },
          { key: "folderid", opt: "EQ", type: "S", value: this.modelid },
          { key: "state", opt: "EQ", type: "S", value: "0" },
        ],
      };
      repositorySpecpage(obj).then((res) => {
        if (!res) return false;
        this.booksList = res.list;
        this.pages.total = res.total;
        if (this.isMore && this.selectedAll) {
          this.selectedCheck();
        }
      });
    },
    submit() {
      if (this.isMore) {
        const data = JSON.parse(JSON.stringify(this.selectedAll));
        this.$emit("confirm", data);
      } else {
        if (this.selectIndex >= 0) {
          const data = JSON.parse(
            JSON.stringify(this.booksList[this.selectIndex])
          );
          this.$emit("confirm", data);
        }
      }
      this.isDialogsVisible = false;
    },
  },
};
</script>

<style lang="scss" scoped>
.listItem {
  background: $active-color;
  color: #fff;
  padding: 10px;
  margin: 0 10px 10px 0;
  display: inline-block;
  text-align: center;
  i {
    cursor: pointer;
  }
}
.search_component {
  margin-bottom: 15px;
  max-width: 240px;
  vertical-align: middle;
  /deep/ .el-input__inner {
    border-color: #2d42af;
  }
  /deep/ .el-input-group__append {
    background-color: #2d42af;
    border-color: #2d42af;
    &:hover {
      background-color: #5768bf;
      border-color: #5768bf;
    }
    span {
      color: #fff;
    }
  }
}
.left_tree_wrapper.el-scrollbar {
  height: 450px !important;
  .custom-tree-node {
    width: calc(100% - 24px);
    display: flex;
    align-items: center;
    box-sizing: border-box;
    p {
      max-width: 100%;
      overflow: hidden;
      text-overflow: ellipsis;
    }
  }
}
.rightPart {
  height: calc(100vh - 160px) !important;
}
</style>
