...
export default {
  ...
  mounted() {
    if (this.toolsData.id) {
      let nodeData = {
        name: this.toolsData.toolname,
        id: this.toolsData.id,
        type: 'tool'
      }
      const nodes = [
        {
      ...
      this.getToolNodeRelation(this.toolsData, nodeData)
    } else {
      this.getData()
    }
  },
    ...
    getData() {
      getNodeRelation(this.$route.query.listid, this.$route.query.keyid).then(res => {
        ...
        if (res) {
          ...
          if (data.repositoryDocDtos && data.repositoryDocDtos.length > 0) {
      ...
      getToolNodeRelation(child.id).then(res => {
        ...
        if (res) {
          ...
          data.forEach(item => {
            ...
            if (item.repositoryDocDtos && item.repositoryDocDtos.length > 0) {
              item.repositoryDocDtos.forEach(child => {
                const list = child.bolist.map(val => {
                  return {
                    name: val.boname,
                    id: val.id,
                    type: 'bo'
                ...
                links.push(
                  { from: item.keyid, to: child.id, color: '#2d42af' }
                )
              })
            }
            if (item.modelListToolsDtos && item.modelListToolsDtos.length > 0) {
              const list = item.modelListToolsDtos.map((child, index) => {
                return {
                  name: child.toolname,
                  id: child.id,
                  type: 'tool'
              ...
              links.push(
                { from: item.keyid, to: list[0].id, color: '#2d42af' }
              )
            }
          })
          this.$set(child, 'ishave', true)
