<template>
  <div id="groupManage">
    ...
    <!--</el-input>-->
    <!--</el-row>-->
    <el-tree v-if="!isSearch" ref="asyncTree" :props="props" :is-leaf="isLeaf" node-key="id" :load="getFilterTreeList"
      :default-expanded-keys="expandedList" :expand-on-click-node="false" lazy>
      <div slot-scope="{ node, data }" class="custom-tree-node">
        ...
        <p class="font14 opertionBtn">
          <el-tooltip :open-delay="1000" class="item" effect="dark" :content="$t('add')" placement="top">
            <i class="el-icon-circle-plus-outline add" @click="handlerTreeOperate('add', data,node)" />
          </el-tooltip>
          <el-tooltip :open-delay="1000" class="item" effect="dark" :content="$t('modify')" placement="top">
            <i v-if="node.level !== 1" class="el-icon-edit add" @click="handlerTreeOperate('edit', data,node)" />
          </el-tooltip>
          <el-tooltip :open-delay="1000" class="item" effect="dark" :content="$t('delete')" placement="top">
            <i v-if="node.level !== 1" class="el-icon-delete del" @click="handlerTreeOperate('del', data,node)" />
          </el-tooltip>
        </p>
      </div>
    </el-tree>
    <el-tree v-if="isSearch" :props="props" :is-leaf="isLeaf" node-key="id" :data="treeData"
      :default-expanded-keys="expandedList" :expand-on-click-node="false" lazy>
      <div slot-scope="{ node, data }" class="custom-tree-node">
        <p class="grow" :title="node.label" @click="nodeClick(node, data)">
          {{ node.label }}
        ...
        <p class="font14 opertionBtn">
          <i class="el-icon-circle-plus-outline add" @click.stop="handlerTreeOperate('add', data,node)" />
          <i v-if="node.level !== 1" class="el-icon-edit add" @click="handlerTreeOperate('edit', data,node)" />
          <i v-if="node.level !== 1" class="el-icon-delete del" @click="handlerTreeOperate('del', data,node)" />
        </p>
      </div>
    ...
    <!-- 添加或删除组织树 -->
    <el-dialog :title="type == 'add' ? $t('add'): $t('modify')" :visible.sync="isDialogsVisible" width="40%">
      <el-form ref="orgRef" label-width="120px" label-position="right" :model="orgForm" :rules="rules">
...
export default {
  ...
  methods: {
    async getFilterTreeList (node, resolve) {
      console.log(node)
      const id = node.data ? node.data.id : '10'
