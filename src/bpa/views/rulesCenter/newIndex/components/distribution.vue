...
export default {
  ...
  watch: {
    value(val) {
      console.log("watch::::");
      console.log(this.disData)//undefined
      if (val) {
    ...
    orginalChecktion: {
      handler: function (val) {
        this.checkData = this.orginalChecktion.map(item => { return item.name }).join(',')
      },
      deep: true,
      immediate: true
    },
  ...
  methods: {
    ...
    grequireTree() {
      ...
      getGroupTreeList({
        ...
        if (this.disData.bpmDto.distribution) {
          let distribution = JSON.parse(this.disData.bpmDto.distribution)
        ...
        const zTree = $.fn.zTree.init($('#chooserOrganizationTree'), self.settings, res)
        // 让第一个父节点展开
        const nodes = zTree.getNodes()
        console.log(nodes)
        if (nodes.length > 0) {
    ...
    onCheck(event, treeId, treeNode) {
      ...
      if (!treeNode.checked && !zTree.getNodesByParam('id', treeNode.pid, null).length) {
        let index = self.orginalChecktion.map(item => item.id).indexOf(treeNode.id)
        if (index !== -1) {
      ...
      for (var i = 0; i < zTree.getCheckedNodes().length; i++) {
        if (zTree.getCheckedNodes()[i].type === '0' && treeNode.id === zTree.getCheckedNodes()[i].id) {
      ...
      if (initArr.type === '0' && !initArr.getCheckStatus().half) {
        ...
        self.orginalChecktion.push({ name: initArr.name})
        return
      } else {
      ...
      this.disData.bpmDto.distribution = JSON.stringify(orglist.name)
    ...
    confirmChoose() {
      ...
      console.log(JSON.parse(self.sendState).orglist.name)
      ...
      for (var m = 0; m < storeArr.length; m++) {
        ...
        if (!!checkNode.length && (checkNode[0].halfCheck || checkNode[0].checked)) {
          tempStore.push(storeArr[m].name)
