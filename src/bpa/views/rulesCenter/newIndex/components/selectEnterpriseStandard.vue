<template>
  <div v-if="isDialogsVisible" id="handbookFiles" class="w100">
    <el-dialog :append-to-body="true" width="1000px" :visible.sync="isDialogsVisible" title="选择企业标准" :close-on-click-modal="false" :close-on-press-escape="false">
      <el-row :gutter="10">
        <el-col :span="6">
          <el-scrollbar class="left_tree_wrapper">
            ...
            >
              <div slot-scope="{ node, data }" class="custom-tree-node">
                ...
                <!-- <p v-if="node.level == 1 || node.level == 2" class="grow" :title="node.label">
                  {{ node.label }}
                </p>
                <p v-else class="grow" :title="node.label">
        ...
        <el-col :span="18">
          ...
          <el-row>
            <el-col :span="12">
              <el-input v-model="keyWords" :placeholder="$t('standardNumber')" :title="$t('standardNumber')" class="search_component" clearable @keyup.13.native="getList">
                <el-button slot="append"  type="primary" @click="pages.pageNum = 1;getList()" >{{$t('search')}}</el-button>
              </el-input>
            </el-col>
          </el-row>
          <!-- 信息表格 -->
          ...
          >
            ...
            <el-table-column prop="foldertype" :label="$t('standardElements')" show-overflow-tooltip />
            <el-table-column prop="title" :label="$t('standardNumber')" show-overflow-tooltip />
            <el-table-column prop="enname" :label="$t('standardName')" show-overflow-tooltip />
            <el-table-column prop="system" :label="$t('standardLevel')" show-overflow-tooltip />
            <el-table-column prop="officicalTitle" :label="$t('standardClassify')" show-overflow-tooltip />
            <el-table-column prop="docnum" :label="$t('adaptableAutomobileMode')" show-overflow-tooltip />
...
export default {
  ...
  watch: {
    value(val) {
      ...
      if (val) {
        this.iechoEcho = true
        this.selectedAll = JSON.parse(JSON.stringify(this.enterpriseStandardSelected))
      } else {
  ...
  methods: {
    ...
    async getFilterTreeList(node, resolve) {
      ...
      // }
      const id = node.data ? node.data.id : '1'
