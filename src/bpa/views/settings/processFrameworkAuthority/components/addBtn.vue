<template>
  <div>
    <el-dialog :title="menuInfo.name" class="person_list" :visible.sync="dialogVisible" width="50%"
      ...
      <el-table :data="personList" class="main_tab" ref="personTable" stripe border>
        ...
        <el-table-column prop="address" :label="$t('operation')" width="80px">
          <template slot-scope="scope">
            ...
            <el-tooltip class="item" effect="dark" :content="$t('delete')" placement="top">
              <el-button class="no-border" @click="deleteBtn(scope.row.id)" icon="el-icon-delete" circle></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-size="pages.size"
        :current-page.sync="pages.page" background layout="prev, pager, next, jumper, total" :total="pages.total">
      </el-pagination>
    ...
    <!-- 添加按钮 -->
    <el-dialog :title="menutitle" :visible.sync="addBtndialogVisible" width="30%" :close-on-click-modal="false"
