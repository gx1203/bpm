    ...
    <div v-for="(item, index) in printCardLen" :key="index">
      ...
      <table cellpadding="0" cellspacing="0">
        <tr>
          ...
          <th colspan="2">外借物资类型:{{ printData.LEND_MATERIAL_TYPE }}</th>
          <th colspan="2">车牌号:{{ printData.PLATE_NUMBER }}</th>
        </tr>
        <tr>
          <th colspan="2">借用单位:{{ printData.CORPORATION }}</th>
          <th colspan="2">外借原因:{{ printData.OUT_REASON }}</th>
...
export default {
  ...
  methods: {
    setPrintCardDatas(obj) {
      ...
      this.printData.APPLY_DATE = nodeData.applyUserDto.applyDate
      ...
      if (typeof nodeData.approvalRecordList != 'undefined') {
        if (approvalRecordList.length > 1) {
          ...
          this.printData.APPROVAL_DATE = this.formatDateTime2(nodeData.approvalRecordList[1].createdate);
        }
      }
