<template>
  <flow-form :node="node"
    ...
    <div slot="approve"
      ...
      <flow-card :data="approveTypeCardItem"
                 :isMoreCard="true">
        <div slot="card">
          <el-row class="form">
            <el-col class="form-item">
              <el-radio-group v-model="submitaction">
                <el-radio v-for="(item, index) in approveTypeList"
                          :label="item.rightscode"
                          :key="index"
                          @change="changeApproveType(item)">{{item.name}}</el-radio>
              </el-radio-group>
            </el-col>
            <el-col :span="8"
                    class="form-item"
              ...
              <el-input clearable type="text"
                        ...
                        reaonly
                        @click.native="peopleDialog = true"></el-input>
              <select-people :isMore="submitaction !== 'transferPackage'"
                             v-model="peopleDialog"
                             @confirm="getUser"></select-people>
            </el-col>
            <el-col class="form-item">
              ...
              <el-button type="primary"
                         ...
                         class="margin-t10">{{ $t('addToCommonApproval') }}</el-button>
              <!-- <el-input clearable type="textarea" placeholder="请填写审批记录..." v-model="approvalComment"></el-input> -->
            </el-col>
          </el-row>
        </div>
      </flow-card>
      <flow-card :data="approvalRecordCardItem"
                 :isMoreCard="true">
        <div slot="card">
          <el-row class="form">
            <el-col :span="24"
                    class="text-r margin-b15">
              <el-button @click="isApprove=!isApprove"
                         type="primary">
            ...
            <el-table v-if="isApprove"
                      :data="approvalRecordList"
                      border
                      style="width: 100%; text-align: center">
              <el-table-column prop="processnodename"
                               :label="$t('node')"
            ...
            <el-col class="approve-table"
                    :span="24"
                    v-else>
              <div class="table-header">
                <sort-button style="flex-grow: 1"
                             :items="headers" />
              </div>
              <div v-for="(item, index) in approvalRecordSectionList"
                   :key="index">
                <h4 v-if="item.approveCategory">{{item.approveCategory}}</h4>
                <div v-for="(val, index1) in item.list"
                     :key="index1">
                  <el-row class="tr-body"
                          type="flex"
                          align="middle">
                    <el-col class="td text-c"
                            :span="headers[0].span">{{val.processnodename}}</el-col>
                    <el-col class="td text-c"
                            :span="headers[1].span">{{val.usernames}}</el-col>
                    <el-col class="td text-c"
                            :span="headers[2].span">{{val.result}}</el-col>
                    <el-col class="td text-c"
                            :span="headers[3].span">{{val.createdate | formatDateTime }}</el-col>
                    <el-col class="td text-c"
                            :span="headers[4].span"
                            :title="val.reason">{{val.reason}}</el-col>
                  </el-row>
                </div>
              </div>
            </el-col>
