                  ...
                  <table>
                    ...
                    <tbody>
                      <tr v-for="(val, index1) in item.PAYLIST" :key="index1">
                        <td>{{ index1 + 1 }}</td>
                        <td>{{ val.STDNAME }}</td>
                        <td>{{ val.NUMBEROFCONFIGURATIONS }}</td>
...
export default {
  ...
  methods: {
    ...
    getFormData() {
      ...
      this.data.PAYLIST.forEach(val => {
        val.PURCHASEUNITPRICE = this.$options.filters.delcommafy(
          val.PURCHASEUNITPRICE
        )
        val.AMOUNTDECLARED = this.$options.filters.delcommafy(
          val.AMOUNTDECLARED
        )
      })
