<template>
  <el-dialog :title="title" id="purchaseProcess" :visible.sync="dialogVisible">
      ...
      <el-row class="form">
        ...
        <el-col :span="12" class="form-item">
          <el-form-item label="申购单价：" prop="PURCHASEUNITPRICE">
            <el-input clearable :disabled="isNoStart"  @focus="textFocus" @blur="doubleFilter" @input="doubleValue" v-model="item.PURCHASEUNITPRICE" min="0"/>
          </el-form-item>
        </el-col>
        <el-col :span="12" class="form-item">
      ...
      <el-row class="form">
        ...
        <el-col :span="12" class="form-item">
          <el-form-item label="采购主管部门：" prop="PROCUREMENTAUTHORITY">
            <el-select clearable :disabled="isNoStart" v-model="item.PROCUREMENTAUTHORITY" style="width: 100%">
              <el-option
...
export default {
  ...
  methods: {
    ...
    textFocus(){
      if (/^\d+(.\d+)?$/.test(this.item.PURCHASEUNITPRICE.replace(/,/g, ""))) {
        this.item.PURCHASEUNITPRICE = this.item.PURCHASEUNITPRICE.replace(/,/g, '')
      }
    },
    doubleValue () {
      this.item.PURCHASEUNITPRICE = this.item.PURCHASEUNITPRICE.replace(/[^0-9.]/g, '')
    },
    doubleFilter(){
       if (this.item.PURCHASEUNITPRICE && this.item.PURCHASEUNITPRICE.indexOf(",") === -1) {
          this.item.PURCHASEUNITPRICE =  this.$options.filters.formatNumber(this.item.PURCHASEUNITPRICE, 2, 1)
