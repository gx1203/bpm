<template>
  <div>
    <flow-form :node="node" :tabsList="tabsList" @handleSubmit="handleSubmit" @handleSave="handleSave" />
    <el-dialog
      title="选择保存表单类型"
...
<script>
import FlowForm from '../flow_form/index'
  ...
  methods: {
    ...
    saveForm () {
      if (this.saveType === '1') {
         if(this.draftName && this.draftName.length > 200){
          this.$message.error('草稿名称长度不能超过200个字符！！')
          return false
        }
        this.$loading()
        saveDraft({
    ...
    handleSubmit (datas) {
       if(this.subject && this.subject.length > 200){
          this.$message.error('主题名称长度不能超过200个字符！！')
          return false
        }
      this.$loading()
      startProcess({
        bizData: Object.assign({},datas),
