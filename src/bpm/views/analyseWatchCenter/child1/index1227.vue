<template>
  <div id="analyseWatchCenter" class="w100">
    <el-row :gutter="10" class="w100 remove-margin-l remove-margin-r">
        ...
        <el-card class="box-card pos-relative">
          ...
          <el-row class="w100 remove-margin-l remove-margin-r">
            <el-col :span="10" class="remove-padding-l">
              <div ref="trainCount" class="h300px" />
            </el-col>
      ...
      <el-col :span="15" class="remove-padding-r">
        <el-card class="box-card">
          <div slot="header" class="clear-fix">
            ...
            <el-button type="text" class="float-r margin-r10" @click="paintExceed">{{ $t('confirm') }}</el-button>
            <span class="float-r">
              <el-date-picker
                v-model="startYear"
                type="month"
          ...
          <template>
            <div v-show="!showExFilter" ref="exTime" class="h300px" />
            <div v-show="showExFilter">
                  ...
                  <el-row>
                    <el-col :span="8">
                      <el-form-item :label="$t('subject')" class="remove-margin-b">
                        <el-input clearable v-model="exSubject" clearable />
                      </el-form-item>
                    </el-col>
                    <el-col :span="8">
                      <el-form-item :label="$t('approver')" class="remove-margin-b">
                        <el-input clearable v-model="exappoval" clearable />
                      </el-form-item>
                    </el-col>
                    <el-col :span="8">
                      <div class="search_buttons_wrap">
                        <el-button type="primary" @click="paintExceed">搜索</el-button>
                        <el-button type="primary" @click="resetExceed">{{ $t('reset') }}</el-button>
                      </div>
                    </el-col>
                  </el-row>
                </el-form>
              ...
              >
                ...
                />
                <el-table-column prop="subject" :label="$t('subject')" show-overflow-tooltip />
                <el-table-column prop="fitter" width="80px" label="发起人" show-overflow-tooltip />
                <el-table-column
                  ...
                  show-overflow-tooltip
                />
                <el-table-column prop="appover" width="80px" :label="$t('approver')" show-overflow-tooltip />
                <el-table-column
                  prop="settime"
                  width="90px"
                  label="规定节点审批时长"
    ...
    <el-row :gutter="10" class="w100 remove-margin-l remove-margin-r margin-t10">
      <el-col :span="12" class="remove-padding-l">
        <el-card class="box-card">
          <div slot="header" class="clear-fix">
            ...
            <el-button type="text" class="float-r margin-r10" @click="paintPblish">{{ $t('confirm') }}</el-button>
            <span class="float-r">
              <el-form class="w150px">
                <el-form-item label="显示条数:" label-width="80px" class="remove-margin-b">
                  <el-input clearable v-model="pblishCount" clearable />
                </el-form-item>
              </el-form>
            </span>
            <el-button
          ...
          <template>
            ...
            <div v-show="!showPblishFilter" ref="pblish" class="h333px" />
            <div v-show="showPblishFilter">
              <el-collapse-transition>
                <el-form
                ...
                >
                  <el-row>
                    <el-col :span="8">
                      <el-form-item label="主题:" label-width="60px" class="remove-margin-b">
                        <el-input clearable v-model="pblishname" clearable />
                      </el-form-item>
                    </el-col>
                  ...
                  <div class="search_buttons_wrap">
                    ...
                    <el-button type="primary" @click="resetpblish">{{ $t('reset') }}</el-button>
                  </div>
                </el-form>
              </el-collapse-transition>
              <el-table
              ...
              >
                <el-table-column :label="$t('number')" width="50px" type="index" />
                ...
                <el-table-column prop="time" width="80px" label="审批通过时间" />
                <el-table-column prop="actualtime" width="80px" label="实际审批时长" sortable="custom" />
                <el-table-column
                  prop="actualavetime"
                  width="100px"
                  label="实际平均审批时长"
      ...
      <el-col :span="12" class="remove-padding-r">
        <el-card class="box-card">
          <div slot="header" class="clear-fix">
            ...
            <el-button type="text" class="float-r margin-r10" @click="paintRate">{{ $t('confirm') }}</el-button>
            <span class="float-r">
              <el-form class="w150px">
                <el-form-item label="显示条数:" label-width="80px" class="remove-margin-b">
                  <el-input clearable v-model="rateCount" clearable />
                </el-form-item>
              </el-form>
            </span>
          </div>
          <template>
            ...
            <div v-show="!showRateFilter" ref="rate" class="h333px" />
            <div v-show="showRateFilter">
              <el-collapse-transition>
                <el-input clearable v-model="ratename":placeholder="$t('placeholderText.pleaseEnterSubject')" class="search_component" clearable>
                  <el-button slot="append" type="primary" @click="paintRate">搜索</el-button>
                </el-input>
              </el-collapse-transition>
              <el-table
                :data="rateData"
              ...
              >
                ...
                <el-table-column
                  ...
                  sortable="custom"
                />
                <el-table-column prop="appvoalcount" width="87px" label="审批个数" sortable="custom" />
                <el-table-column prop="exceedcount" width="87px" label="超期个数" sortable="custom" />
                <el-table-column prop="exceedrate" width="90px" label="超期率(%)" sortable="custom" />
              </el-table>
            </div>
          </template>
        </el-card>
    ...
    <el-dialog :title="trainproc" :visible.sync="dialogTableVisible" width="70%">
      <el-table :data="trainprocesslist">
        ...
        <el-table-column property="code" :label="$t('processCoding')" width="80" show-overflow-tooltip />
        <el-table-column property="name" :label="$t('processName2')" width="150" show-overflow-tooltip />
        <el-table-column property="version" :label="$t('version')" width="50" show-overflow-tooltip />
        <el-table-column property="name" :label="$t('state')" width="80">
          <template slot-scope="scope">
            <div>{{ scope.row.status ==='2'?'已培训':'未培训' }}</div>
          </template>
        </el-table-column>
        <el-table-column property="bponame" label="流程负责人" show-overflow-tooltip />
        <el-table-column property="upbponame" label="上级流程负责人" width="150" show-overflow-tooltip />
        <el-table-column property="effectivetime" :label="$t('releaseTime')" width="100" show-overflow-tooltip />
    ...
    <el-dialog :title="pblishdetailtit+' 详情'" :visible.sync="dialogpblishVisible" width="70%">
      <el-table :data="pblishdetails">
        ...
        <el-table-column property="nodename" :label="$t('approvalNodeName')" width="150" show-overflow-tooltip />
        <el-table-column property="owner" :label="$t('approver')" width="150" show-overflow-tooltip />
        <el-table-column property="settime" label="规定节点审批时长" show-overflow-tooltip />
        <el-table-column property="actualtime" label="实际节点审批时长" width="150" show-overflow-tooltip />
        <el-table-column property="overdueday" label="超期天数" width="100" show-overflow-tooltip />
      </el-table>
    </el-dialog>
    <el-dialog :visible.sync="dialograteVisible" title="详情" width="70%">
      <el-table :data="ratedetails">
        <el-table-column type="index" width="50" />
        <el-table-column property="timerange" label="审批时间范围" show-overflow-tooltip />
        <el-table-column property="name" :label="$t('name')" width="100" show-overflow-tooltip />
        <el-table-column property="setavetime" label="规定平均审批时长" width="150" show-overflow-tooltip />
        <el-table-column
          ...
          show-overflow-tooltip
        />
        <el-table-column property="appvoalcount" label="审批个数" width="100" show-overflow-tooltip />
        <el-table-column property="exceedcount" label="超期个数" width="100" show-overflow-tooltip />
...
export default {
  ...
  computed: {
        ...
        tooltip: {
          ...
          formatter(tooltipData) {
            return ` ${tooltipData[0].name}<br/>
                   审批人 : ${self.exData[tooltipData[0].dataIndex].appover}<br/>
                   发起人 : ${self.exData[tooltipData[0].dataIndex].fitter}<br/>
                   超期天数 : ${self.exData[tooltipData[0].dataIndex].exceedtime}天<br/>
                   ${tooltipData[1].marker}实际节点审批时长 : ${tooltipData[1].value}天<br/>
                   ${tooltipData[0].marker}规定节点审批时长 : ${tooltipData[0].value}天`
          }
        },
        grid: {
          left: '4%',
    ...
    pblishOptions() {
      ...
      const self = this
      const data = self.pblishData
      const index = self.showNodeIndex
      const xdata = []
      let xRoseData = []
      const ydataBar = []
      const ydataLine = []
  ...
  data[tooltipData[0].dataIndex]
    ...
    rateOptions() {
      ...
      const self = this
      const data = self.rateData
      const index = self.showrateNodeIndex
      const xdata = []
  ...
  mounted() {
    ...
    new Promise(async(resolve, reject) => {
      // 最近发布流程
      await this.paintPblish()
      // 审批效率
      await this.paintRate()
      resolve()
    }).then(() => {
      // 定时器
      if (this.ratetimer) {
      ...
      if (this.pblishtimer) {
        clearInterval(this.pblishtimer)
      }
      this.setTimer()
    })
  },
  ...
  methods: {
    ...
    paintTrain() {
      const self = this
      const trainCount = echarts.init(self.$refs.trainCount)
      getTrained().then(res => {
        self.trainShow = res
        self.trainData = [
    ...
    paintExceed() {
      const self = this
      const exTime = echarts.init(self.$refs.exTime)
    ...
    async paintPblish() {
      const self = this
      const pblish = echarts.init(self.$refs.pblish)
      ...
      const res = await getPblish(self.pblishparam)
      self.pblishData = res
    ...
    async paintRate() {
      const self = this
      const rate = echarts.init(self.$refs.rate)
      ...
      const res = await getRate(self.rateparam)
      self.rateData = res
      if (self.rateOptions) {
    ...
    setTimer() {
      ...
      if (this.pblishOptions) {
        this.pblishtimer = setInterval(() => {
    ...
    raterendering() {
      ...
      const rate = echarts.getInstanceByDom(this.$refs.rate)
      rate.setOption(this.rateOptions, true)
    },
    // 给echatrs添加点击事件,改变当前点击柱状图的背景色
    addrateClick() {
      const eChart = echarts.getInstanceByDom(this.$refs.rate)
      this.$refs.rate.onmouseover = () => {
        if (this.ratetimer) {
          clearInterval(this.ratetimer)
        }
        return false
      }
      this.$refs.rate.onmouseout = () => {
        if (this.ratetimer) {
          clearInterval(this.ratetimer)
        }
        if (this.pblishtimer) {
    ...
    pblishrendering() {
      ...
      const pblish = echarts.getInstanceByDom(this.$refs.pblish)
      pblish.setOption(this.pblishOptions, true)
    },
    // 给echatrs添加点击事件,改变当前点击柱状图的背景色
    addpblishClick() {
      const eChart = echarts.getInstanceByDom(this.$refs.pblish)
      this.$refs.pblish.onmouseover = () => {
        if (this.pblishtimer) {
          clearInterval(this.pblishtimer)
        }
        return false
      }
      this.$refs.pblish.onmouseout = () => {
        if (this.ratetimer) {
          clearInterval(this.ratetimer)
        }
        if (this.pblishtimer) {
