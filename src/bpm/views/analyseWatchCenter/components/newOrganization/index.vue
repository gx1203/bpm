<template>
  <div id="organization">
    <el-dialog :visible.sync="dialogVisible" :before-close="handleDialogClose" title="选择操作" width="60%">
      <el-row class="header">
        <!-- 组织类型选择 -->
        <el-col :span="12">
      ...
      <el-row>
        <el-col :span="10">
          <el-input clearable v-model="searchText" class="search" placeholder="请输入内容" prefix-icon="el-icon-search"/>
        ...
        <el-col :span="10" :offset="4" class="transfer">
          ...
          <el-transfer
            ...
            <el-button v-show="showMore == true" slot="left-footer" @click="loadMore">更多</el-button>
          </el-transfer>
        </el-col>
      </el-row>
      <div slot="footer" class="dialog-footer">
        <el-button @click="cancelSelectOrg">{{ $t('cancelAction') }}</el-button>
    ...
    <el-dialog :title="groupTitle" :visible.sync="addGroupVisible" :show-close="false" width="320px">
      <el-form ref="groupForm" :model="groupForm" :rules="addGroupRules">
        <el-form-item v-if="groupTitle == '自定义组'" prop="name" label="组名：" label-width="70px">
          <el-input clearable v-model="groupForm.name" autocomplete="off"/>
        </el-form-item>
        <el-form-item v-else-if="groupTitle == '指定组'" prop="groupId" label="选择组：" label-width="80px">
          <el-select clearable v-model="groupForm.groupId" placeholder="请选择组">
            <el-option
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="cancelAddGroup('groupForm')">{{ $t('cancelAction') }}</el-button>
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (backupData.length == 0) {
            this.$confirm('此组没有添加成员，是否确定添加此组?', this.$t('hintText.hint'), {
