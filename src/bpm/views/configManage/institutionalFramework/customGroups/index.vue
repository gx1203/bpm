<template>
  <el-card id="internationalConfig" shadow="never">
    <div class="tool-common-wrap">
      ...
      <div class="btn-wrap">
        <el-button type="primary" @click="companyHandle($t('addToCustomGroup'))">
          {{ $t('add') }}
          <!---->
        </el-button>
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item :label="$t('organizationType') + '：'">
            <el-select clearable v-model="searchForm.owner" class="w100"
    ...
    <div class="tableWrap" v-loading="loading">
        ...
        <div v-if="content.list && content.list.length!==0">
          <div class="tr" v-for="(item, index) in content.list" :key="index">
            ...
            <el-row class="tr-body" type="flex" align="middle">
              ...
              <el-col class="td" :span="headers[2].span">{{filterType(item.owner)}}</el-col>
              <el-col style="text-align: center" :span="headers[3].span">
                <el-tooltip class="item" :content="$t('edit')">
                  <el-button type="text" icon="el-icon-edit" @click="companyHandle($t('editCustomGroup'),item)">
                  </el-button>
                </el-tooltip>
      ...
      <div v-else>
        <el-table :data="content.list" class="margin-t10" stripe :header-cell-style="{background: '#E1EAFD !important'}"
          ...
          <el-table-column :label="$t('organizationType')" show-overflow-tooltip sortable prop="owner">
            <template slot-scope="scope">
              <div>{{filterType(scope.row.owner)}}</div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('operation')" width="100">
            <template slot-scope="scope">
              <el-tooltip class="item" :content="$t('edit')">
                <el-button type="text" icon="el-icon-edit" @click="companyHandle($t('editCustomGroup'),scope.row)">
                </el-button>
              </el-tooltip>
              <el-tooltip class="item" :content="$t('delete')">
    ...
    <custom-groups-dialog v-model="dialogVisible" :itemId="editItem.id" :title="linkTitle" @confirm="saveSuccess" />
  </el-card>
</template>
<script>
import customGroupsDialog from "./customGroupsDialog";
...
export default {
  ...
  mounted () {
    getDictionaryOption({ dictionaryen: "自定义组-组织类型" }).then(rt => {
  ...
  methods: {
    ...
    companyHandle (item, data) {
      this.linkTitle = item;
      if (item === this.$t('addToCustomGroup')) {
        this.editItem = {
          hrtype: '10'
        };
        this.dialogVisible = true;
      } else if (item === this.$t('editCustomGroup')) {
