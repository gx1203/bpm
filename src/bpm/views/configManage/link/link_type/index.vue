<template>
  <el-card shadow="never">
    ...
    >
      <el-row>
        <el-col :span="8">
          <el-form-item :label="$t('linkTypeID') + '：'">
            <el-input clearable type="text" v-model="searchForm.link_Type_Code"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="8">
          <el-form-item :label="$t('linkTypeName') + '：'">
            <el-input clearable type="text" v-model="searchForm.link_Type_Name"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
    ...
    <div v-loading="loading">
      <div v-if="$store.state.app.tableStyle ==='default'">
        ...
        <div class="tr" v-for="(item, index) in content.list" :key="index">
          ...
          <el-row class="tr-body" type="flex" align="middle">
            <el-col class="td" :span="headers[0].span">{{item.linkTypeCode}}</el-col>
            <el-col class="td" :span="headers[1].span">{{item.linkTypeName}}</el-col>
            <el-col style="text-align: center" :span="headers[2].span">
              <el-tooltip class="item" effect="dark" :content="$t('editLinkType')" placement="top">
                <el-button type="text" @click="addList($t('editLinkType'),item)">{{ $t('editLinkType') }}</el-button>
              </el-tooltip>
      ...
      <div v-else>
        ...
        >
          ...
          <el-table-column :label="$t('linkTypeID')" show-overflow-tooltip sortable prop="linkTypeCode">
            <template slot-scope="scope">
              <div>{{scope.row.linkTypeCode}}</div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('linkTypeName')" show-overflow-tooltip sortable prop="linkTypeName">
            <template slot-scope="scope">
              <div>{{scope.row.linkTypeName}}</div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('operation')"  width="100">
            <template slot-scope="scope">
              <el-tooltip class="item" effect="dark" :content="$t('editLinkType')" placement="top">
                <el-button type="text" @click="addList($t('editLinkType'),scope.row)">{{ $t('editLinkType') }}</el-button>
              </el-tooltip>
            </template>
          </el-table-column>
        </el-table>
  ...
  data () {
    return {
      ...
      headers: [
        {
          key: 'linkTypeCode',
          name: this.$t('linkTypeID'),
          span: 10
        },
        {
          key: 'linkTypeName',
          name: this.$t('linkTypeName'),
          span: 10
        },
        {
  ...
  methods: {
    ...
    handleDelete (item) {
      this.$confirm(this.$t('hintText.areYouSureDelete'), this.$t('hintText.hint'), {
        ...
        .then(() => {
          deleteLinkType(item.id).then(rt => {
            if (rt.status === '200') {
              this.getData()
              this.$message({
                message: this.$t('hintText.successfullyDelete'),
    ...
    addList (item, data) {
      if (item === '添加链接类型') {
        ...
        this.linkTitle = item
        this.editItem = {}
      } else if (item === this.$t('editLinkType')) {
        this.dialogVisible = true
