<template>
  <el-tabs v-model="activeName" type="card">
    <el-tab-pane v-for="(item, index) in data" :key="item.key" :label="item.tabname" :name="item.tabname" :disabled="isPrint">
      <flow-card1 v-for="(item2, index2) in item.nodeTabLists.filter(el => el.istab === '0')" :data="item2" :key="item2.key" :is-print="isPrint" />
      <flow-tabs1 v-for="(item2, index2) in tabsList[index]" :data="item2" :key="item2.key" v-if="item.istab === '1'" :is-print="isPrint" />
    </el-tab-pane>
  </el-tabs>
</template>
