<template>
  <div>
    <el-popover trigger="manual" v-model="cityPanelIsShow">
      <div class="vcp-panel">
        <h5>{{ $t('placeholderText.citySelection') }}</h5>
        <ul class="vcp-tags">
        ...
        <div class="vcp-list">
          <div :class="{'z-hot': initial == 'hot'}" v-for="(citys, initial, index) in cityList" :key="index"
      ...
      <el-input clearable slot="reference" readonly :disabled="disabled" :placeholder="placeholder"
        v-model="currCityName" @click.native="cityPaneClick" />
    </el-popover>
  </div>
