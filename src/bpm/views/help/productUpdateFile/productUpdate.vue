<template>
  <el-card shadow="never">
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item :label="$t('releaseTime')+'：'">
            ...
            <el-date-picker v-model="searchForm.rtime" type="daterange" value-format="yyyy-MM-dd HH:mm:ss"
      ...
      <el-table :data="content.list" class="margin-t10" stripe :header-cell-style="{background: '#E1EAFD !important'}"
        ...
        <el-table-column :label="$t('operating')" width="120">
          <template slot-scope="scope">
            ...
            <el-tooltip class="item" content="下载">
              <el-button type="text" @click="productDownload(scope.row)">下载</el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 弹出下拉选择框 -->
    <product-download :item="itemData" :version="versionData" v-model="dialogVisible" @confirm="saveSuccess">
    </product-download>
  </el-card>
</template>
...
import { getFieldLibraryList, deleteFieldLibraryList, deleteAllFieldLibraryList, getProcessVersionList } from '@/bpm/api/process/advancedConfig/field_maintenance.js'
...
// 数据接口
import { getProductUpdateList, ProductUpdateDownload, getProductVersion } from '@/bpm/api/help/onlineHelp/help'
import { getPost } from "@/bpm/api/process/administrativeTool/agents_change";
import { getApplyList, getShortCutList, addToShortCut, postionList, delShortCut } from "@/bpm/api/staging/personal/apply_create";
...
export default {
  ...
  methods: {
    ...
    productDownload (item) {
      ...
      this.itemData = item
      console.log(item);
      getProductVersion().then((rt) => {
        this.versionData = rt.data
