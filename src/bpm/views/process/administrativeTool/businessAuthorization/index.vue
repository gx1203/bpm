<template>
  <el-card shadow="never" id="stagingToolBusinessAuth">
    ...
    <div class="item-right"  v-show="processInfo.type === '1'">
      ...
      <el-form class="power-search" :model="searchForm" @keyup.enter.native="searchGetData" ref="searchForm"
      label-width="110px" size="mini" v-if="isPowerSearch"> 
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item :label="$t('authNode') + '：'">
              <el-select  clearable v-model="searchForm.nodename" class="w100"
          ...
          <el-col :span="12"> 
            <el-form-item :label="$t('agent') + '：'">
              <el-input clearable type="text" v-model="searchForm.agentusercn"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12">  
            <el-form-item :label="$t('judgmentLogic') + '：'">
               <el-select  clearable v-model="searchForm.judgemethod" class="w100"
              :placeholder="$t('placeholderText.pleaseSelect')"> 
                <el-option v-for="item in judgmentLogicList" :key="item.value" :label="item.name" :value="item.value"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
          <!-- <el-col :span="8">  
           ...
           <el-col :span="12">  
            <el-form-item :label="$t('state') + '：'">
               <el-select  clearable v-model="searchForm.state" class="w100"
              :placeholder="$t('placeholderText.pleaseSelect')"> 
        ...
        <el-row>
          <el-col :span="24" class="text-r">
            ...
            <el-button type="primary" @click="searchGetData">{{ $t('query') }}</el-button>
            <el-button type="primary" @click="searchForm = {};searchGetData()">{{ $t('reset') }}</el-button>
          </el-col>
        </el-row>
      </el-form>
      <div v-loading="loading">
          ...
          <div v-if="content.list && content.list.length!==0">
            <div class="tr" v-for="(item, index) in content.list" :key="index">
              ...
              <el-row  class="tr-body" type="flex" align="middle">
                <el-col class="td" :span="headers[0].span">{{item.nodename}}</el-col>
                <el-col class="td" :span="headers[1].span">{{item.agentusercn}}</el-col>
                <el-col class="td" :span="headers[2].span">{{item.judgemethod}}</el-col>
                <el-col class="td" :span="headers[3].span">{{item.judgeparam}}</el-col>
                <el-col class="td" :span="headers[4].span">{{item.begintime}}</el-col>
                <el-col class="td" :span="headers[5].span">{{item.endtime}}</el-col>
                <el-col class="td" :span="headers[6].span">{{item.recovertime}}</el-col>
                <el-col class="td" :span="headers[7].span">{{item.state}}</el-col>
        ...
        <el-table :data="content.list" class="margin-t10" stripe
            ...
            <el-table-column :label="$t('authorizer')" show-overflow-tooltip prop="authorizedusercn"  min-width="100"></el-table-column>
            <el-table-column :label="$t('authNode')" show-overflow-tooltip prop="nodename"  min-width="100"></el-table-column>
            <el-table-column :label="$t('agent')" show-overflow-tooltip prop="agentusercn"></el-table-column>
            <el-table-column :label="$t('judgmentLogic')" show-overflow-tooltip prop="judgemethod"  min-width="100"> 
               <template slot-scope="scope">
                 {{ scope.row.judgemethod | judgmentLogicFilter }}
               </template>
            </el-table-column>  
            <el-table-column :label="$t('judgmentConditions')" show-overflow-tooltip prop="judgeparam"  min-width="150"></el-table-column>
            <el-table-column :label="$t('startTime')" show-overflow-tooltip prop="begintime"  min-width="120">
               <template slot-scope="scope">
                 {{ scope.row.begintime | formatDateTimess }}
               </template>
            </el-table-column>
            <el-table-column :label="$t('endTime')" show-overflow-tooltip prop="endtime" min-width="120">
               <template slot-scope="scope">
                 {{ scope.row.endtime | formatDateTimess }}
               </template>
            </el-table-column>
            <el-table-column :label="$t('takeTheTime')" show-overflow-tooltip prop="recovertime" min-width="120">
               <template slot-scope="scope">
                 {{ scope.row.recovertime | formatDateTimess }}
               </template>
            </el-table-column>
            <el-table-column :label="$t('state')" show-overflow-tooltip prop="state" min-width="50"> 
               <template slot-scope="scope">
            ...
            <el-table-column :label="$t('operation')" width="90">
              <template slot-scope="scope">
                ...
                <el-button type="text" v-show="scope.row.state === '0'" @click="handleCancel(scope.row)">
                  <i class="iconfont icon-weibiaoti545" style="font-size: 10px"/>
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        @current-change="searchType === 'power' ? getData() : quickQueryData()"  :total="total">
       </el-pagination>
      </div>
    </div>
      <add-business-auth-dialog v-model="dialogVisible" :item="editItem" :processInfo="processInfo"  :title="linkTitle"
      @confirm="saveSuccess" />
  </el-card>
</template>
...
import { getAllTree, queryList,queryListAll, updateState, getAuthNodeData, deleteAuth, batchDelete,getAuthorizedUser } from '@/bpm/api/staging/tool/business_auth.js'
        ...
        type: 'warning'
          ...
          updateState({ 
            ...
            state:"1"
              ...
              if(res.status === '200'){
                this.getTableData(this.processInfo.id)
                this.$message.success(this.$t("hintText.successfulAuthorizationCancellation"));
              }else{
    ...
    searchGetData(){
      ...
      this.filters = [
        ...
        }, 
        {
          ...
          'opt': 'BETWEEN',
          'type': 'D',
          'value': this.searchForm.begintime?this.searchForm.begintime.join('~'):''
        }, {
          'key': 'endtime',
          'opt': 'BETWEEN',
          'type': 'D',
          'value': this.searchForm.endtime?this.searchForm.endtime.join('~'):''
        },
        {
          'key': 'recovertime',
          'opt': 'BETWEEN',
          'type': 'D',
          'value': this.searchForm.recovertime?this.searchForm.recovertime.join('~'):''
        },
        {
          'key': 'state',
          'opt': 'LIKE',
