<template>
  <el-card shadow="never">
    <div class="tool-common-wrap">
      ...
      <div class="btn-wrap">
        <el-button type="primary" @click="addList('添加移动模块配置')">
          {{ $t('add') }}
          <!---->
        </el-button>
    ...
    <div class="tr" v-for="(item, index) in content.list" :key="index">
      ...
      <el-row class="tr-body" type="flex" align="middle">
        ...
        <el-col style="text-align: center" :span="headers[2].span">
          <el-button type="text" icon="el-icon-edit" @click="addList('编辑移动模块配置',item)"></el-button>
  ...
  methods: {
    handleDeleteOne(item) {
      this.$confirm(this.$t('hintText.areYouSureDelete'), this.$t('hintText.hint'), {
        ...
        .then(() => {
          deleteMobileGroup(item.menuid).then(rt => {
            if (rt.status === "200") {
                this.getData();
              this.$message({
