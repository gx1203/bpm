    ...
    <div v-loading="loading" class="tableWrap">
      ...
      <div v-else>
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div>{{categoryAction(scope.row)}}</div>
              <!-- <div>
                {{
          ...
          >
            <template slot-scope="scope">
              <div>
                <span
                  :class="{
                    timeout: scope.row.timeoutState === 'Y',
                    'will-timeout': scope.row.timeoutState === 'L'
                  }"
                >
                  {{ scope.row.remainingDate }}
                </span>
                <span v-if="!scope.row.remainingDate">
                  {{ $t('noTimeLimit') }}
                </span>
              </div>
            </template>
  ...
  methods: {
    ...
    categoryAction (info) {
      if (info.category === 'TRANSFER') {
        return this.$t('administrativetodo')
      } else if (info.category === 'MORE_APPROVE') {
        return this.$t('communicationtodo')
      } else if (info.category === 'ReturnTodo' || info.category === 'MORE_APPLY' || info.category === 'RETRIAL') {
        return this.$t('returnTodo')
      }
      return this.$t('normaltodo')
    },
    // 页码改变事件
